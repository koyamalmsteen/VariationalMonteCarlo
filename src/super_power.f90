!
! $Id: super_power.f90,v 1.4 2003/11/20 14:04:31 k-yukino Exp $
!

#include "parameter.h"

subroutine super_power(alpha_psi0,alpha_psi1,alpha_h_psi1,alpha_h_h_psi1,&
...
!! <super> 
        call super(alpha_psi0,vector_up,vector_down,&
             i_count,j_count,tmp_result,name,error)
        call error_check(name,error)

        tmp_result=tmp_result*0.25*0.25
...
!! <H super H>
        call super_nume3_1(alpha_h_psi1,vector_up,vector_down,i_count,j_count,&
             result23,name,error)
        call error_check(name,error);

        tmp_result3_1=result23*alpha_h_psi1
        ...
        if( POWER==2 ) then
!! <O H^2>   ---> <psi|O|alpha><alpha|H^2|psi>とする (<super>の結果を使う)
...
!! <H^2 O H^2> ---> <psi|H^2|alpha><alpha|OH^2|psi>
           call super_nume5(alpha_h_h_psi1,vector_up,vector_down,&
