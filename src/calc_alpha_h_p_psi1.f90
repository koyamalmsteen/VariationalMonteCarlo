...
subroutine calc_alpha_h_p_psi1(vector_up,vector_down,input,tmp_jastrow,result,&
     name,error)
...
! 要素計算と内積
                 ...
                 if( electric_field/=0 .and. PERIODIC==1 .and. xi_field/=0 ) then
                    result_up=result_up+dble(TRANSFER)*q_up*input&
                         *jastrow(gamma_up,gamma_down)*calc_total_pole(gamma_up,gamma_down)
!!!*lambda_field(gamma_up,gamma_down)
!!
!! koko ha kyousei 0 desu f(0)
!!
                 else
                    result_up=result_up+dble(TRANSFER)*q_up*input&
...
! 要素計算と内積
                 ...
                 if( electric_field/=0 .and. PERIODIC==1 .and.xi_field/=0 ) then
                    result_down=result_down+dble(TRANSFER)*q_down*input&
                         *jastrow(gamma_up,gamma_down) &
                         *calc_total_pole(gamma_up,gamma_down)
!!                         *lambda_field(gamma_up,gamma_down)
!! koko ha kyousei 0 f(0)
!!
                 else
                    result_down=result_down+dble(TRANSFER)*q_down*input&
                         *jastrow(gamma_up,gamma_down)
...
! まずダブルオキュパイの数を調べる。
  ...
  result_coulomb=dble(COULOMB)*double_occupy*input*tmp_jastrow*calc_total_pole(gamma_up,gamma_down)
...
! まずPを求める
     ...
     result_field=electric_field*pole*input*tmp_jastrow*calc_total_pole(gamma_up,gamma_down)
  end if

  result=result_up+result_down+result_coulomb+result_field

end subroutine calc_alpha_h_p_psi1
