...
real(8) function calc_total_pole(gamma_up,gamma_down)
...
! local
  integer,dimension(TOTAL_SITE_NUMBER) :: site_table_up 
  integer,dimension(TOTAL_SITE_NUMBER) :: site_table_down
  integer :: i_count
  real(8) :: tmp_result
  integer :: qn 
  integer :: max_x
! init
  tmp_result=0
  site_table_up=0 ; site_table_down=0
  max_x=0

! サイト表示に
  do i_count=1,TOTAL_UP_ELECTRON
     site_table_up(gamma_up(i_count))=1
  end do
  do i_count=1,TOTAL_DOWN_ELECTRON
     site_table_down(gamma_down(i_count))=1
  end do

  do i_count=1,TOTAL_SITE_NUMBER
! up電子があれば
     if( site_table_up(i_count)==1 .and. site_table_down(i_count)==1 ) then
        qn=-1
     else if( site_table_up(i_count)==1 .and. site_table_down(i_count)==0 ) then
        qn=0
     else if( site_table_up(i_count)==0 .and. site_table_down(i_count)==1 ) then
        qn=0
     else
        qn=1
     end if

     tmp_result=tmp_result+xaxis(i_count)*qn
  end do

  calc_total_pole=tmp_result

end function calc_total_pole
